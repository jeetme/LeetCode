class Solution {
    ...
    return areRowsValid(board) && areColsValid(columns) && areMiniBoxesValid(miniBoxes);

    }

    boolean isValid(char[] element) {
        for(int i = 0; i < element.length - 1; i ++) {
            if(element[i] != '.' && contains(element, element[i], i + 1)) {
                return false;
            }
        }
        return true;
    ...
    boolean contains(char[] array, char target, int start) {
    ...
    boolean areRowsValid(char[][] board) {
        boolean result = true;
        for(int i = 0; i < 9; i ++) {
            result = result && isValid(board[i]);
        }
        return result;
    }

    boolean areColsValid(char[][] columns) {
        boolean result = true;
        for(int i = 0; i < 9; i ++) {
            result = result && isValid(columns[i]);
        }
        return result;
    }

    boolean areMiniBoxesValid(char[][] miniBoxes){
        boolean result = true;
        for(int i = 0; i < 9; i ++) {
            result = result && isValid(miniBoxes[i]);
        }
        return result;
    }    
}